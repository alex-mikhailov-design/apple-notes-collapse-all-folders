...
macOS Tahoe has a bug where Apple Notes expands every folder and subfolder in the sidebar on every app launch. Apple hasn't fixed it as of macOS 26.5 (May 2026). Some of the Apple Community threads about the bug:
...
I have not been able to find a ready-to-use solution. So I made this Apple Shortcut, that uses AppleScript to collapse all folders back in one click. This shortcut works with both iCloud notes and On My Mac notes.

> [!NOTE]
> This shortcut only works on macOS. It relies on AppleScript and System Events, which are not available on iOS or iPadOS.
...
<img src="assets/demo.gif" width="100%" alt="Demo Animation">
...
> [!IMPORTANT]
> 1. Before running, go to **"System Settings > Privacy & Security > Accessibility"** and enable **"Shortcuts"**. The script simulates keyboard input, so macOS requires this permission.
> 2. The Notes app must be running, and the sidebar must be visible when you run the Shortcut. If it's hidden, toggle it 
> with **"Option ⌥ + Command ⌘ + S"** keyboard shortcut or through the menu **"View > Show Folders"**.
...
5. Run the shortcut from the Shortcuts app or a widget.

### Option 2 - Manually
1. Open the Shortcuts app on your Mac.
2. Create a new shortcut.
3. Add a **Text** block and type `2, 10` or your own values, see [Shortcut Configuration](#shortcut-configuration) below.
4. Add a **Run AppleScript** action and paste the code from the [AppleScript](#applescript) section below.
5. Save the shortcut.
6. Run the shortcut from the Shortcuts app or a widget.

<br>

> [!TIP]
> **Add Shortcut as a Desktop Widget**
> 
...
## How It Works
The AppleScript in the shortcut uses System Events to simulate **Option ⌥ + Left Arrow ◀** keyboard shortcut in the Apple Notes sidebar to collapse a folder and all its subfolders, and other keyboard shortcuts for navigation within the sidebar.
